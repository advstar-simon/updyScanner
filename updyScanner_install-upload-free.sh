...
${PACKAGE_ATTRIBUTES}
    ...
    echo "$JSON" > "scanResults_${PROFILENAME}.json"
}
...
function scan {
    ...
    if [ "${PACKAGE_ATTRIBUTES}" != "unknown" ]; then
        ...
        log "Scan completes"
        log "========================================================"
        log " Please upload ./scanResults_${PROFILENAME}.json to updy.io manually "
        log "========================================================"
    fi
}
